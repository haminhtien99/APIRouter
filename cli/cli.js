...
// Subcommands (`apirouter xai video …`) run against an already-running gateway
...
// The standalone package bundles sql.js. Native SQLite and tray helpers remain
// opt-in and are installed under cli/.runtime, never in a global environment.
...
// Identifiers for killAllAppProcesses - only kill APIRouter specifically
...
for (let i = 0; i < args.length; i++) {
  if (args[i] === "--port" || args[i] === "-p") {
    ...
    // Kept as a no-op for compatibility with existing local launch scripts.
...
Options:
  ...
  --skip-update       Accepted for compatibility; APIRouter never self-updates
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running apirouter/cli.js, or next-server.
            // Avoids killing unrelated processes that merely mention the project.
            const cmd = line.toLowerCase();
            const isAppProcess =
              cmd.includes("node") && cmd.includes("apirouter") &&
              (cmd.includes("cli.js") || cmd.includes("\\apirouter") || cmd.includes("/apirouter"));
            if (isAppProcess) {
              const match = line.match(/^"(\d+)"/);
        ...
        try {
          ...
          lines.forEach(line => {
            ...
            const isAppProcess =
              cmd.includes("node") && cmd.includes("apirouter") &&
              (cmd.includes("cli.js") || cmd.includes("/apirouter"));
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
...
const { createTelegramBotController } = require("./src/cli/telegramBot");
const telegramBot = createTelegramBotController({
  appDir: standaloneDir,
...
async function showInterfaceMenu() {
  ...
  const serverUrl = endpoint.replace(/\/v1$/, "");
  ...
  menuItems.push(
    ...
    {
      label: `Telegram Bot: ${telegramBot.getStatus().running ? "ON" : "OFF"} → toggle`,
      icon: "✈️"
    },
...
function startServer() {
  ...
  // Surface real network exposure only when explicitly bound to all interfaces.
  if (host === "0.0.0.0") {
  ...
  function spawnServer() {
    ...
    const child = spawn(RUNTIME, ["--dns-result-order=ipv4first", "--max-old-space-size=6144", serverPath], {
      ...
      env: {
        ...
        PORT: port.toString(),
        HOSTNAME: host,
        APIROUTER_LAUNCHER_PID: process.pid.toString()
      }
    });
  ...
  if (trayMode) {
    ...
    waitServerReady(port).then(() => {
      if (process.env.APIROUTER_INTERNAL_TELEGRAM_BOT_ENABLED === "1") {
        const result = telegramBot.start();
        if (!result.success) console.error(`⚠️  ${result.error}`);
      }
      initTrayIcon();
    ...
    try {
      while (true) {
        ...
        if (choice === "web") {
          ...
          const result = telegramBot.toggle();
          const { pause } = require("./src/cli/utils/input");
          if (result.success) {
            console.log(`\n✈️  Telegram Bot is now ${result.status.running ? "ON" : "OFF"}.`);
          } else {
            console.log(`\n❌ ${result.error}`);
          }
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 APIRouter is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          const bgProcess = spawn(process.execPath, ["--dns-result-order=ipv4first", __filename, "--tray", "--skip-update", "-p", port.toString()], {
            ...
            env: {
              ...process.env,
              APIROUTER_INTERNAL_TELEGRAM_BOT_ENABLED: telegramBot.getStatus().running ? "1" : "0"
            }
          });
          bgProcess.unref();

          console.log(`🔔 APIRouter is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "apirouter", "db.json") : path.join(".apirouter", "db.json"));
