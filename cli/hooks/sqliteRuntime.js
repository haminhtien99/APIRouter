// Ensure better-sqlite3 is installed in this checkout's .runtime/node_modules.
...
function getDataDir() {
  ...
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || os.homedir(), "apirouter")
    : path.join(os.homedir(), ".apirouter");
}

function getRuntimeDir() {
  return process.env.APIROUTER_RUNTIME_DIR || path.resolve(__dirname, "..", ".runtime");
}
...
function ensureRuntimeDir() {
  ...
  if (!fs.existsSync(pkgPath)) {
    fs.writeFileSync(pkgPath, JSON.stringify({
      ...
      version: "1.0.0",
      private: true,
      description: "Optional local runtime dependencies for APIRouter",
    }, null, 2));
  }
  return dir;
}
