// Lazy install systray2 for macOS/Linux into cli/.runtime/node_modules.
...
function cleanupLegacySystray({ silent = false } = {}) {
  // 1) Runtime dir: cli/.runtime/node_modules/systray
  // 2) npm global nested: <npm_prefix>/node_modules/apirouter/node_modules/systray
  ...
  for (const dir of targets) {
    if (fs.existsSync(dir)) {
      try {
        ...
        if (!silent) console.log(`[apirouter][runtime] removed legacy systray: ${dir}`);
      } catch (e) {
        if (!silent) console.warn(`[apirouter][runtime] failed to remove ${dir}: ${e.message}`);
      }
    }
  }
}
...
function chmodSystrayBin({ silent = false } = {}) {
  ...
  try {
    ...
    if (!silent) console.warn(`[apirouter][runtime] chmod tray bin failed: ${e.message}`);
  }
}
