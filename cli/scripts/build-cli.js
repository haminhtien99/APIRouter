...
const cliAppDir = process.env.APIROUTER_CLI_APP_DIR || path.join(cliDir, "app");
...
function resolveStandaloneBuild(appDir, buildDistDir) {
  ...
  // NEXT_TRACING_ROOT_MODE=workspace, e.g. standalone/APIRouter/server.js.
...
function buildCliPackage() {
  console.log("📦 Building APIRouter CLI package with Next.js...\n");
  ...
  const bundledPkgPath = path.join(cliAppDir, "package.json");
  const bundledPkg = JSON.parse(fs.readFileSync(bundledPkgPath, "utf8"));
  bundledPkg.name = "apirouter-app";
  bundledPkg.description = "APIRouter local AI account and provider gateway";
  bundledPkg.version = cliPkg.version;
  bundledPkg.private = true;
  ...
  // Step 3a.1: Copy Telegram bot files used by the launcher ON/OFF control.
  copyTelegramBotRuntime(appDir, cliAppDir);
  console.log("✅ Copied Telegram bot runtime\n");

  // Step 3b: Ensure sql.js (pure JS fallback) bundled in app/cli/app/node_modules.
  // Strip better-sqlite3 (native); the optional local runtime owns native bindings.
  ...
  if (fs.existsSync(betterDir)) {
    ...
    console.log("✅ Stripped better-sqlite3 (optional local runtime dependency)");
  }
  console.log("");
