...
const CLI_TOKEN_SALT = "apirouter-cli-auth";
const APP_NAME = "apirouter";
 ...
 * @returns {Promise<Object>} { success, data: { installed, hasAPIRouter, ... } }
 */
...
/**
 * Refresh a CLI tool's generated model catalog (PUT)
 * @param {string} tool - Tool name
 * @returns {Promise<Object>} { success, data }
 */
async function syncCliToolModels(tool) {
  return makeRequest("PUT", `/api/cli-tools/${tool}-settings`);
}
...
async function getQuotaConnections({ provider = null, activeOnly = true, limit = 20 } = {}) {
  const params = new URLSearchParams({
    page: "1",
    pageSize: String(Math.max(1, Math.min(Number(limit) || 20, 100))),
    accountStatus: activeOnly ? "active" : "all",
    sort: "priority",
  });
  if (provider) params.set("provider", provider);
  return makeRequest("GET", `/api/providers/client?${params.toString()}`);
}

async function getConnectionQuotaSummary(connectionId, { force = false } = {}) {
  const query = force ? "?force=1" : "";
  return makeRequest("GET", `/api/usage/${encodeURIComponent(connectionId)}/summary${query}`);
}
