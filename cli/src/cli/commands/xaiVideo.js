/**
 * `apirouter xai video` — generate a Grok Imagine video through the local
 * apirouter gateway and save the result as an MP4 file.
 *
...
const HELP = `
Usage: apirouter xai video --prompt "..." [options]

Generate a Grok Imagine video via your local apirouter gateway
...
Options:
  ...
  --api-key <key>         APIRouter API key (or env APIROUTER_API_KEY)
...
function parseArgs(argv) {
  const opts = {
    ...
    port: DEFAULT_PORT,
    host: DEFAULT_HOST,
    apiKey: process.env.APIROUTER_API_KEY || null,
...
async function run(argv) {
  ...
  try {
    ...
    const connectionId = create.headers["x-apirouter-connection-id"] || null;
