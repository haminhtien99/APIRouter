...
async function buildCodexHeader() {
  ...
  const { installed, hasAPIRouter, config } = result.data;
...
async function codexQuickSetup(port) {
  ...
  // Get model selection
  const model = await selectModelFromList("Select Codex Model", "cx/claude-sonnet-4-5-20250929", { excludeCombos: false });
  if (!model) return;
...
async function codexSyncModels() {
  const result = await api.syncCliToolModels("codex");
  const count = result.data?.modelCatalog?.modelCount;
  const successMessage = Number.isFinite(count)
    ? `Synced ${count} models and combos to native /model. Restart Codex to reload.`
    : "Native /model catalog synced. Restart Codex to reload.";
  showStatus(result.success ? successMessage : `Failed: ${result.error}`, result.success ? "success" : "error");
  await pause();
}
...
async function showCodexMenu(port, breadcrumb = []) {
  await showMenuWithBack({
    ...
    items: [
      ...
      {
        label: "🔄 Sync Native /model",
        action: async () => { await codexSyncModels(); return true; }
      },
      {
        label: "Reset to Default",
...
async function buildDroidHeader() {
  ...
  const { installed, hasAPIRouter, settings } = result.data;
  ...
  // Extract APIRouter custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:APIRouter-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract APIRouter provider config
  const provider = settings?.models?.providers?.["apirouter"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("apirouter/") ? primary.replace("apirouter/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasAPIRouter, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use APIRouter\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
