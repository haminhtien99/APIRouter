...
const COMBO_STRATEGIES = [
  { value: "fallback", label: "Fallback — try models in order" },
  { value: "round-robin", label: "Round Robin — rotate models" },
  { value: "fusion", label: "Fusion — panel responses + judge" },
];

function strategyLabel(strategy) {
  return COMBO_STRATEGIES.find((item) => item.value === strategy)?.label || COMBO_STRATEGIES[0].label;
}

function buildUpdatedComboStrategies(comboStrategies, comboName, strategy, judgeModel = "") {
  ...
  if (strategy === "fusion") {
    next.judgeModel = judgeModel || "";
  } else {
    delete next.judgeModel;
    delete next.fusionTuning;
  }
  updated[comboName] = next;
  return updated;
}

async function loadComboStrategies() {
  const result = await api.getSettings();
  if (!result.success) return { success: false, error: result.error, strategies: {} };
  return { success: true, strategies: result.data.comboStrategies || {} };
}

async function saveComboStrategy(comboName, strategy, judgeModel = "") {
  const current = await loadComboStrategies();
  if (!current.success) return current;
  const comboStrategies = buildUpdatedComboStrategies(current.strategies, comboName, strategy, judgeModel);
  const result = await api.updateSettings({ comboStrategies });
  return result.success ? { success: true, strategies: comboStrategies } : result;
}

async function chooseComboStrategy(current = "fallback") {
  const defaultIndex = Math.max(0, COMBO_STRATEGIES.findIndex((item) => item.value === current));
  const index = await select("Select combo strategy:", COMBO_STRATEGIES.map((item) => item.label));
  return COMBO_STRATEGIES[index >= 0 ? index : defaultIndex].value;
}

async function chooseFusionJudge(currentJudge = "") {
  const choice = await select("Select Fusion judge:", [
    "Auto — use the first combo model",
    currentJudge ? `Keep current — ${currentJudge}` : "Choose a model",
...
async function handleChangeStrategy(combo, currentStrategy = {}) {
  const strategy = await chooseComboStrategy(currentStrategy.fallbackStrategy || "fallback");
  const judgeModel = strategy === "fusion"
    ? await chooseFusionJudge(currentStrategy.judgeModel || "")
    : "";
  const result = await saveComboStrategy(combo.name, strategy, judgeModel);
  showStatus(
    result.success ? `Strategy updated to ${strategyLabel(strategy)}` : `Strategy update failed: ${result.error}`,
...
async function showComboActions(combo, breadcrumb = []) {
  ...
  await showMenuWithBack({
    ...
    refresh: async () => {
      const result = await loadComboStrategies();
      return { strategy: result.strategies[combo.name] || {} };
    },
    headerContent: ({ strategy }) => {
      const current = strategy.fallbackStrategy || "fallback";
      const judge = current === "fusion" ? (strategy.judgeModel || `Auto — ${formatModel(combo.models?.[0] || "first model")}`) : null;
      return [
        `Name: ${combo.name}`,
        `Strategy: ${strategyLabel(current)}`,
    ...
    items: [
      {
        label: ({ strategy }) => `Change Strategy (${strategy.fallbackStrategy || "fallback"})`,
        action: async ({ strategy }) => {
          await handleChangeStrategy(combo, strategy);
          return true;
        }
      },
      {
...
async function handleEditSingleCombo(combo) {
  ...
  if (result.success) {
    if (name !== combo.name) {
      const current = await loadComboStrategies();
      if (current.success && current.strategies[combo.name]) {
        const migrated = { ...current.strategies, [name]: current.strategies[combo.name] };
        delete migrated[combo.name];
        await api.updateSettings({ comboStrategies: migrated });
      }
    }
...
async function handleDeleteSingleCombo(combo) {
  ...
  if (confirmed) {
    ...
    if (result.success) {
      const current = await loadComboStrategies();
      if (current.success && current.strategies[combo.name]) {
        const updated = { ...current.strategies };
        delete updated[combo.name];
        await api.updateSettings({ comboStrategies: updated });
      }
...
async function showCombosMenu(breadcrumb = []) {
  ...
  await showListMenu({
    ...
    breadcrumb,
    fetchItems: async () => {
      const [result, settingsResult] = await Promise.all([api.getCombos(), api.getSettings()]);
      if (!result.success) {
        clearScreen();
      ...
      const strategies = settingsResult.success ? (settingsResult.data.comboStrategies || {}) : {};
      const items = (result.data.combos || []).map((combo) => ({
        ...combo,
        _strategy: strategies[combo.name]?.fallbackStrategy || "fallback",
      }));
      return { items };
    },
    formatItem: (combo) => {
      ...
      const displayModels = modelsChain.length > maxLen 
        ...
        : modelsChain;
      return `${combo.name} [${combo._strategy || "fallback"}]: ${displayModels}`;
    },
...
async function handleCreateCombo() {
  ...
  const strategyResult = await saveComboStrategy(name, strategy, judgeModel);
  if (!strategyResult.success) {
    showStatus(`Combo created, but strategy could not be saved: ${strategyResult.error}`, "warning");
    await pause();
    return;
  }
  
  showStatus(`Combo "${name}" created with ${strategyLabel(strategy)}!`, "success");
  await pause();
}
