...
const { showQuotaMenu } = require("./menus/quota");
...
function renderHeader(port, keys) {
  const lines = [];
  lines.push(`Endpoint: ${COLORS.green}http://127.0.0.1:${port}/v1${COLORS.reset}`);
  lines.push(`Mode:     ${COLORS.dim}local only${COLORS.reset}`);
...
function getHeader(port) {
  ...
  return cachedHeader || `Endpoint: http://127.0.0.1:${port}/v1\nMode:     ${COLORS.dim}local only${COLORS.reset}\nKey:      ${COLORS.dim}...${COLORS.reset}`;
}
...
async function startTerminalUI(port) {
  ...
  await showMenuWithBack({
    ...
    items: [
      ...
      {
        label: "Quota / Token Limits",
        action: async () => {
          await showQuotaMenu([...basePath, "Quota / Token Limits"]);
          return true;
        }
      },
      {
