...
const APP_LABEL = "com.apirouter.autostart";
 ...
 *   2. `process.argv[1]` if it's our cli.js — true when apirouter is currently
 ...
 * SIGTERM to the running process. When the running apirouter cli.js was itself
...
<dict>
    ...
    <string>/tmp/apirouter.log</string>
    <key>StandardErrorPath</key>
    <string>/tmp/apirouter.error.log</string>
</dict>
</plist>`;
...
function enableWindows(cliPath) {
  ...
  // `apirouter.cmd` lookup that depended on the npm prefix path.
...
Type=Application
Name=APIRouter
Comment=APIRouter local AI gateway
