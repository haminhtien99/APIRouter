...
// Per-process secret proving x-ar-real-ip was stamped below rather than sent by the client.
// A bare `next start` / `next dev` never loads this file, so it cannot produce a matching
// header even though the env var is inherited by child processes. Named like x-ar-cli-token
...
http.createServer = (...args) => {
  ...
  const wrapped = (req, res) => {
    ...
    delete req.headers["x-ar-real-ip"];
    delete req.headers["x-forwarded-for"];
    delete req.headers["x-ar-via-proxy"];
    delete req.headers["x-ar-peer-token"];
    req.headers["x-ar-real-ip"] = ip;
    req.headers["x-ar-peer-token"] = PEER_TOKEN;
    if (viaProxy) req.headers["x-ar-via-proxy"] = "1";
