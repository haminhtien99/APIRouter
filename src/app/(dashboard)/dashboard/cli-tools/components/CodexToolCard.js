...
export default function CodexToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  const handleApplySettings = async () => {
    ...
    setMessage(null);
    try {
      // Use sk_apirouter for localhost if no key, otherwise use selected key
      const keyToUse = (selectedApiKey && selectedApiKey.trim())
        ? selectedApiKey
        : (!cloudEnabled ? "sk_apirouter" : selectedApiKey);
  ...
  const handleSyncModels = async () => {
    ...
    setMessage(null);
    try {
      const res = await fetch("/api/cli-tools/codex-settings", { method: "PUT" });
      const data = await res.json();
      if (res.ok) {
        setMessage({
          type: "success",
          text: `${data.message}. Restart Codex to reload native /model.`,
        });
        await checkCodexStatus();
      } else {
        setMessage({ type: "error", text: data.error || "Failed to sync native /model" });
      }
    } catch (error) {
      setMessage({ type: "error", text: error.message });
    } finally {
...
[model_providers.apirouter.http_headers]
        ...
        <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
          ...
          {!checkingCodex && codexStatus?.installed && (
            <>
              ...
              {codexStatus?.modelCatalog?.exists && (
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleSyncModels} disabled={syncingModels || !codexStatus?.hasAPIRouter} loading={syncingModels}>
                  <span className="material-symbols-outlined text-[14px] mr-1">sync</span>Sync Native /model
                </Button>
