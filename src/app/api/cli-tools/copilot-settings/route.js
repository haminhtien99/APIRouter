...
const hasAPIRouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "APIRouter");
};

const getAPIRouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "APIRouter") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getAPIRouterEntry(config);
...
// POST - Apply APIRouter config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing APIRouter entry or append
    const idx = config.findIndex((e) => e.name === "APIRouter");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove APIRouter entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "APIRouter");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "APIRouter removed from Copilot config",
    });
  } catch (error) {
