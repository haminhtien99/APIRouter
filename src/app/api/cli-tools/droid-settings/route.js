...
// Check if settings has APIRouter customModels
const hasAPIRouterConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:APIRouter"));
};
...
// POST - Update APIRouter customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing APIRouter configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:APIRouter"));
...
// DELETE - Remove APIRouter customModels only (keep other settings)
