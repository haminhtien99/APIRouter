...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-apirouter.env");
};
...
const hasAPIRouterConfig = (config) => {
  ...
  if (providers["apirouter"]) return true;

  for (const [name, provider] of Object.entries(providers)) {
    if (provider.base_url && provider.base_url.includes("localhost:20228")) {
      return true;
    }
  }
...
export async function GET() {
  ...
  const hasAPIRouter = hasAPIRouterConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_APIROUTER_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile apirouter",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["apirouter"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_APIROUTER_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "apirouter configuration removed from jcode",
    });
  } catch (error) {
