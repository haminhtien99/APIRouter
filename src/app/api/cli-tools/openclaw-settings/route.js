...
// Check if settings has APIRouter config
const hasAPIRouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["apirouter"];
};

// Read per-agent models.json and return current model id (without "apirouter/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["apirouter"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasAPIRouter: hasAPIRouterConfig(settings),
...
// POST - Update APIRouter settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `apirouter/${model}`;

    // Remove all old apirouter/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("apirouter/"))
    ...
    // Add fresh apirouter models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`apirouter/${m}`] = {};
    });

    // Remove old apirouter model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("apirouter/")) {
    ...
    // Update models.providers.apirouter with all models
    settings.models.providers["apirouter"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `apirouter/${agentModel}` };
        return agent;
      });
...
export async function DELETE() {
  try {
    ...
    // Remove APIRouter from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["apirouter"];
    ...
    // Remove apirouter models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("apirouter/"));
    ...
    // Reset agents.defaults.model.primary if it uses apirouter
    if (settings.agents?.defaults?.model?.primary?.startsWith("apirouter/")) {
