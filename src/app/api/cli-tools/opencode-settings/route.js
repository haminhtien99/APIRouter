...
const hasAPIRouterConfig = (config) => {
  ...
  return !!config.provider["apirouter"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["apirouter"];
    ...
    return NextResponse.json({
      ...
      configPath: getConfigPath(),
        opencode: {
          ...
          activeModel: config?.model?.startsWith("apirouter/") ? config.model.replace(/^apirouter\//, "") : null,
...
// POST - Apply APIRouter as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_apirouter";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing apirouter provider entry and its models
    const existingProvider = config.provider["apirouter"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["apirouter"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `apirouter/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `apirouter/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("apirouter/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove APIRouter provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["apirouter"]?.models) {
      delete config.provider["apirouter"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["apirouter"].models).length === 0) {
        delete config.provider["apirouter"];
        if (config.model?.startsWith("apirouter/")) delete config.model;
      } else if (config.model === `apirouter/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["apirouter"].models);
        config.model = `apirouter/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire apirouter provider
      if (config.provider) delete config.provider["apirouter"];
      if (config.model?.startsWith("apirouter/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("apirouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "APIRouter settings removed from OpenCode",
    });
  } catch (error) {
