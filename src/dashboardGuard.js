...
function isLoopbackPeer(request) {
  if (hasTrustedPeerHeaders(request)) {
    return isLoopbackHostname(request.headers.get("x-ar-real-ip"));
  }
...
export function isLocalRequest(request) {
  ...
  if (request.headers.get("x-ar-via-proxy")) return false;
...
async function hasValidToken(request) {
  const token = request.cookies.get(AUTH_COOKIE_NAMES.dashboard)?.value;
