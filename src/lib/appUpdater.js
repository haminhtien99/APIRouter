...
// Collect PIDs from this APIRouter installation only (excluding current).
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        const isAppProcess = lower.includes("apirouter") &&
          (lower.includes("cli.js") || lower.includes("\\cli\\app\\") || lower.includes("/cli/app/"));
        if (isAppProcess) {
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const lower = line.toLowerCase();
        const isAppProcess = lower.includes("apirouter") &&
          (lower.includes("cli.js") || lower.includes("/cli/app/"));
        if (isAppProcess) {
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "apirouter");
  }
...
// Resolve npx/apirouter binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx apirouter` — works regardless of global bin path changes after npm i -g
