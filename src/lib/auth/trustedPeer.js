// x-ar-real-ip is only trustworthy when custom-server.js stamped it from the TCP socket.
...
export function hasTrustedPeerHeaders(request) {
  const token = process.env.APIROUTER_PEER_TOKEN;
  return Boolean(token) && request.headers.get("x-ar-peer-token") === token;
}
