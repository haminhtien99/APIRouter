...
function createTempDir() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "apirouter-cli-build-"));
...
describe("CLI build server artifacts", () => {
  it("copies the Telegram bot runtime into the packaged app", () => {
    const root = createTempDir();
    const appDir = path.join(root, "apirouter");
    const cliAppDir = path.join(root, "cli-app");
    writeFixture(appDir, "scripts/telegram-bot.mjs", "bot runtime");
    writeFixture(appDir, "scripts/telegram-bot-lib.mjs", "bot library");

    copyTelegramBotRuntime(appDir, cliAppDir);

    assert.equal(fs.readFileSync(path.join(cliAppDir, "scripts/telegram-bot.mjs"), "utf8"), "bot runtime");
    assert.equal(fs.readFileSync(path.join(cliAppDir, "scripts/telegram-bot-lib.mjs"), "utf8"), "bot library");
  });
