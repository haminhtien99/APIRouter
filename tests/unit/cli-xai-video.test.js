/**
 * Tests for the `apirouter xai video` CLI command (cli/src/cli/commands/xaiVideo.js)
 *
 * Uses a real local HTTP server standing in for the apirouter gateway + video CDN.
    ...
    ({ server } = await startServer((req, res) => {
      if (req.method === "POST" && req.url === "/v1/videos/generations") {
        ...
        req.on("end", () => {
          ...
          res.writeHead(200, { "Content-Type": "application/json", "x-apirouter-connection-id": "conn-42" });
