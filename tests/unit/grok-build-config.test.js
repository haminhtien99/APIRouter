...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("apirouter");
    ...
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "apirouter-general-purpose",
      explore: "apirouter-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\.apirouter\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.apirouter-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.apirouter-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# apirouter-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model.apirouter-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("apirouter-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model.apirouter-");
    expect(reset).not.toContain("apirouter-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("apirouter-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("apirouter-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("apirouter-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("apirouter-explore");
    expect(getGrokSubagentSlot("plan")).toBe("apirouter-plan");
