...
// path, so x-ar-real-ip arrives straight from the client and a remote caller can claim to
...
describe("peer header trust", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    process.env.APIROUTER_PEER_TOKEN = PEER_TOKEN;
  ...
  afterEach(() => {
    ...
    delete process.env.APIROUTER_PEER_TOKEN;
  });
...
describe("login limiter client IP", () => {
  ...
  it("buckets spoofed peer IPs together so lockout cannot be rotated away", () => {
    const first = getClientIp(request("/api/auth/login", { "x-ar-real-ip": "1.1.1.1" }));
    const second = getClientIp(request("/api/auth/login", { "x-ar-real-ip": "2.2.2.2" }));
  ...
  it("still honours TRUST_PROXY for operators fronting apirouter with a reverse proxy", () => {
